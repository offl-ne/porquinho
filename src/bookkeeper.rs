...
use crate::error::{Error, Result, TomlTypeCheck, TomlTypeCheckDiagnosis};
...
impl Bookkeeper {
    ...
    pub fn load_from_path(path: &Path) -> Result<Self> {
        ...
        let (table, type_check_diagnosis) = Self::load_toml_table_or_default(&file_contents);
        ...
        if type_check_diagnosis.has_error_description() {
            ...
            Ok(Self { file, file_contents, table })
        }
    }
    ...
    fn load_toml_table_or_default(input_text: &str) -> (TomlTable, TomlTypeCheckDiagnosis) {
